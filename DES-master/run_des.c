...
int hex_char_to_byte(char hex_char) {
...
void hex_string_to_bytes(const char* hex_string, unsigned char* byte_array, int byte_array_size) {
    for (int i = 0; i < byte_array_size; i++) {
        int high_nibble = hex_char_to_byte(hex_string[i * 2]);
        int low_nibble = hex_char_to_byte(hex_string[i * 2 + 1]);
...
int main(int argc, char* argv[]) {
    ...
    if (strcmp(argv[1], ACTION_GENERATE_KEY) == 0) { // Generate key file
        ...
        number_of_blocks = file_size/16 + ((file_size%16)?1:0); // Each block is 16 hex characters (8 bytes)

        start = clock();

        // Start reading input file, process and write to output file
        char hex_block[17]; // 16 hex characters + null terminator
        while(fgets(hex_block, 17, input_file)) {
            block_count++;
            hex_block[16] = '\0'; // Ensure null termination
            hex_string_to_bytes(hex_block, data_block, 8);

            if (block_count == number_of_blocks) {
                if (process_mode == ENCRYPTION_MODE) {
                    padding = 8 - (file_size / 2) % 8;
